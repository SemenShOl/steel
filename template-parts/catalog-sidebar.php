<aside class="catalog-sidebar">
    ...
    <?php
        ...
        function findObjectByHref($array, $href) {
            foreach ($array as $item) {
                if (isset($item['href']) && trim($item['href'], '/') === trim($href, '/')) {
                    return $item;
                }
                if (isset($item['childrenCategoryParsed'])) {
                    $result = findObjectByHref($item['childrenCategoryParsed'], trim($href, '/'));
                    if ($result !== null) {
                        return $result; 
                    }
                }
        ...
        function findTopParentId($array, $childId) {
            $itemData = findObjectById($array, $childId);

            if (isset($itemData['parentCategory']) && $itemData['parentCategory'] !== false) {
                return findTopParentId($array, $itemData['parentCategory']);
            }
            ...
            foreach ($allData as $key => $parentCat) {
                ...
                if ($parentCat['childrenCategory'] && count($parentCat['childrenCategory']) > 0) {
                    foreach ($parentCat['childrenCategory'] as $key => $subcat) {
                        ...
                        if (trim($subcatData['href'], '/') == trim($pageSlug, '/')) {
                            $subcatNewData['active'] = true;
                            $data['active'] = true; 
                        } else {
                            $currentItem = findObjectByHref($allData, $pageSlug);
                            $parentId = findTopParentId($allData, $currentItem['id']);

                            if ($parentCat['id'] == findTopParentId($allData, $currentItem['id'])) {
                                $data['active'] = true; 
                            }
                        }
